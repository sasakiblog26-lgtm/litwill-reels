...
```jsonc
{
  ...
  "orientation": "portrait"       // "portrait"(縦1080x1920・既定) | "landscape"(横1920x1080)。省略時 portrait
}
```

### orientation（動画の向き）
`orientation` で縦・横を切り替えます（**省略時は `portrait`**＝従来どおり）。全テンプレ（single/ranking/edu）で指定可能です。

| 値 | 解像度 | 出力ファイル名 | Instagram 投稿 |
|----|--------|----------------|----------------|
| `portrait`（既定） | 1080×1920（縦） | `output/<date>.mp4` | 通常どおり投稿 |
| `landscape` | 1920×1080（横） | `output/<date>-landscape.mp4` | **自動スキップ**（リールは 9:16 のみ） |

- `landscape` はリール（9:16 必須）に投稿できないため、`publish-instagram.mjs` が `--orientation landscape` で **自動的に投稿をスキップ**します（動画は生成・commit され、公開 URL は作られます）。YouTube 横長などへの手動アップロード用途を想定。
- 横動画は 5〜10 分尺になり合成に時間がかかるため、CI の `timeout-minutes` は 45 分です。
- 既存の queue JSON（`orientation` なし）の挙動・出力は変わりません。
...
## 動画の仕様
- 既定 1080×1920（縦）。`orientation: "landscape"` で 1920×1080（横）/ H.264 high / yuv420p / AAC 128k / 30fps
